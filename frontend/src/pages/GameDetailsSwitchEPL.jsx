  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 flex flex-col">
      ...
      <div className="flex-1">
        ...
        {currentView === 3 && (
          <ScorersView boxscore={boxscore} awayTeam={awayTeam} homeTeam={homeTeam} getTeamLogo={getTeamLogo} />
        )}
      </div>
    </div>
  );
      ...
      <div className="bg-gray-800 rounded-3xl p-8 shadow-2xl">
        ...
        {statCategories.map((cat, i) => (
          ...
          >
            ...
            <div className="text-center text-2xl text-gray-300 font-semibold">{cat.label}</div>
...
const ScorersView = ({ boxscore, awayTeam, homeTeam, getTeamLogo }) => {
  const awayTeamData = boxscore?.teams?.find(t => t.team.id === awayTeam?.id);
  const homeTeamData = boxscore?.teams?.find(t => t.team.id === homeTeam?.id);

  const awayScorers = awayTeamData?.scoringSummary || [];
  const homeScorers = homeTeamData?.scoringSummary || [];

  const awayKeepers = awayTeamData?.goalieSummary || [];
  const homeKeepers = homeTeamData?.goalieSummary || [];
