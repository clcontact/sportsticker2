  ...
  const fetchData = async () => {
    try {
      const [summaryRes, statsRes] = await Promise.all([
        fetch(`https://site.api.espn.com/apis/site/v2/sports/basketball/${league}/scoreboard/${gameId}`),
        fetch(`https://site.api.espn.com/apis/site/v2/sports/basketball/${league}/summary?event=${gameId}`)
      ]);
        ...
        {[awayTeam, homeTeam].map((team) => {
          ...
          return (
            ...
            >
              ...
              {renderTimeoutDots(team.timeoutsRemaining || 0)}
            </div>
          );
        })}
      </div>
...
const TeamLeadersView = ({ statsData, competition, targetTeam, awayTeam, homeTeam }) => {
  ...
  const topCategories = ['points', 'assists', 'rebounds'];
...
export default GameDetailsSwitchNBA;